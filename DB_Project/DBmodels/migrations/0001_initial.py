# Generated by Django 3.1.4 on 2020-12-19 06:15

from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    initial = True

    dependencies = [
    ]

    operations = [
        migrations.CreateModel(
            name='Course',
            fields=[
                ('Cno', models.IntegerField(primary_key=True, serialize=False)),
                ('Cname', models.CharField(max_length=20, unique=True)),
                ('credit', models.IntegerField(default=2)),
                ('v', models.IntegerField(default=3)),
            ],
        ),
        migrations.CreateModel(
            name='Department',
            fields=[
                ('Dno', models.IntegerField(primary_key=True, serialize=False)),
                ('Dname', models.CharField(max_length=20, unique=True)),
            ],
        ),
        migrations.CreateModel(
            name='Teacher',
            fields=[
                ('Tno', models.IntegerField(primary_key=True, serialize=False)),
                ('Tname', models.CharField(max_length=20)),
            ],
        ),
        migrations.CreateModel(
            name='TC',
            fields=[
                ('id', models.IntegerField(primary_key=True, serialize=False)),
                ('Cno', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='DBmodels.course')),
                ('Dno', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='DBmodels.department')),
                ('Tno', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='DBmodels.teacher')),
            ],
        ),
        migrations.CreateModel(
            name='Student',
            fields=[
                ('Sno', models.IntegerField(primary_key=True, serialize=False)),
                ('Sname', models.CharField(max_length=20)),
                ('Dno', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='DBmodels.department')),
            ],
        ),
        migrations.CreateModel(
            name='SC',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('free', models.BooleanField(default=False)),
                ('end', models.BooleanField(default=False)),
                ('grade', models.IntegerField(default=60)),
                ('Cno', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='DBmodels.course')),
                ('Sno', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='DBmodels.student')),
                ('TC', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='DBmodels.tc')),
            ],
        ),
    ]
